...
def purge_old_events(conn: sqlite3.Connection, retention_days: int = 30) -> None:
    ...
    try:
        ...
        conn.commit()
        print(f"[Retention] Purged {process_deleted} process records and {network_deleted} network records older than {retention_days} days")
    except Exception as e:
        print(f"[Retention] [Error] Could not delete events from database: {e}")

def vacuum_database() -> None:
    print("[Retention] Running vacuum...")
    try:
        with sqlite3.connect(DB_PATH) as conn:    
            conn.execute("vacuum")
            print("[Retention] Vacuum complete")
    except Exception as e:
        print(f"[Retention] [Error] Vacuum failed: {e}")
